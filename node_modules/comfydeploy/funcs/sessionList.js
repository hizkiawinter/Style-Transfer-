 ...
 */
async function sessionList(client, request, options) {
    const parsed = (0, schemas_js_1.safeParse)(request, (value) => operations.GetMachineSessionsSessionsGetRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    const payload = parsed.value;
    const body = null;
    const path = (0, url_js_1.pathToFunc)("/sessions")();
    ...
    const [result] = await M.match(M.json(200, z.array(components.GPUEventModel$inboundSchema)), M.jsonErr(422, errors.HTTPValidationError$inboundSchema), M.fail(["4XX", "5XX"]))(response, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
