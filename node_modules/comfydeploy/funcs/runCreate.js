 ...
 */
async function runCreate(client, request, options) {
    const input = request;
    const parsed = (0, schemas_js_1.safeParse)(input, (value) => operations.PostRunRequestBody$outboundSchema.optional().parse(value), "Input validation failed");
    if (!parsed.ok) {
        return parsed;
    }
    ...
    const [result] = await M.match(M.json(200, operations.PostRunResponseBody$inboundSchema), M.fail([401, "4XX", "5XX"]), M.jsonErr(500, errors.PostRunResponseBody$inboundSchema))(response, { extraFields: responseFields });
    if (!result.ok) {
        return result;
    }
    return result;
}
//# sourceMappingURL=runCreate.js.map